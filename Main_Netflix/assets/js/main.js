(function (jQuery) {
    ...
    $(document).on('keyup', '.search-input', function (e) {
        var _this = this,
            ...
            _movieContainer = document.querySelector('.movies-search-container');

        _movieContainer.innerHTML = '';
        _movieContainer.classList.add('hide');

        if (searchValue.length >= 3) {
            $.ajax({
                url: "includes/searchMovies.php",
                type: 'post',
                data: {
                ...
                success: function (data) {
                    ...
                    if (dataObj.movies.length > 0) {
                        _movieContainer.classList.remove('hide');
                    }

                    dataObj.movies.forEach(function (_movie) {
                        const link = _movie.tipo === 'filme' ? 'inf_filmes.php' : 'inf_series.php';
                        _movieContainer.insertAdjacentHTML('beforeend', `
                            <a href="${link}?titulo=${encodeURIComponent(_movie.titulo)}" class="movie-item">
                                <div class="imagem" style="background-image: url(${_movie.imagem_url});"></div>
                                <div class="titulo">${_movie.titulo}</div>
                            </a>
                        `);
                    });                                      
                },
